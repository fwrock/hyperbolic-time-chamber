...
/** Example demonstrating Kafka-based dynamic routing.
  *
  * This example shows:
  *   1. LinkActors publishing traffic conditions to Kafka 2. Route calculation using dynamic
  *      weights from local cache 3. Performance comparison with static routing
  */
object KafkaDynamicRoutingExample {
      ...
      linkId =>
        ...
        DynamicWeightCache.publishCost(dynamicCost, ttlSeconds = 300) match {
          case scala.util.Success(_) =>
            ...
            println(
              s"  Published $linkId: $status (congestion=${congestion}x, cost=${dynamicCost.totalCost})"
            )
  ...
  def showCacheStatistics(): Unit = {
    ...
    println(
      f"  Lookup time: ${batchTime}%.3f ms (${batchTime * 1000 / linkWeights.size}%.1f μs per link)"
    )

    dynamicWeights.foreach {
      case (linkId, weight) =>
        val staticWeight = linkWeights(linkId)
        val factor = weight / staticWeight
        val status =
          if (factor > 3.0) "🚨" else if (factor > 1.5) "🔴" else if (factor > 1.1) "🟡" else "🟢"
