...
object KafkaTopicNaming {
  ...
  def getTopicConfig(topicName: String): TopicConfig = {
    topicName match {
      ...
      case Routing.DynamicCosts => TopicConfig(
        name = topicName,
        partitions = 12,
        replicationFactor = 1,
      ...
      case Mobility.VehicleUpdates => TopicConfig(
        name = topicName,
        partitions = 8,
        replicationFactor = 1,
      ...
      case System.ActorLifecycle | System.PerformanceMetrics => TopicConfig(
        name = topicName,
        partitions = 4,
        replicationFactor = 1, 
  ...
  def printTopicConfiguration(): Unit = {
    ...
    topics.foreach { topic =>
      ...
      println(f"   Partitions: ${config.partitions}%2d | Replication: ${config.replicationFactor} | Retention: ${config.retentionMs/60000}%3d min")
    }
