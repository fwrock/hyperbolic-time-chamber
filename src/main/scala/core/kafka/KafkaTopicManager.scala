 ...
 * 
 * Ensures all required topics exist with proper configuration before simulation starts.
 * Uses AdminClient to create topics if they don't exist.
 */
object KafkaTopicManager {
  ...
  /**
   * Initialize all HTC topics.
   * Call this during application startup before any Kafka producers/consumers.
   */
  def initializeAllTopics(): Try[Unit] = {
    Try {
      ...
      val topicsToCreate = allTopics.map { topicName =>
      ...
      case Some("list") =>
        KafkaTopicManager.listExistingTopics() match {
          case Success(topics) =>
            ...
            topics.toSeq.sorted.foreach(topic => println(s"   - $topic"))
          case Failure(e) =>
