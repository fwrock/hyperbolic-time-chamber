...
case class Properties(
  ...
  /** Gets the time manager by type.
    ...
    * @return
    *   The time manager ActorRef, or None if not found
    */
  ...
  /** Gets the default time manager. Priority order:
    *   1. Time manager of defaultTimeManagerType from timeManagers map 2. "discrete-event" time
    *      manager from timeManagers map (fallback)
    *
    * @return
    *   The default time manager ActorRef, or null if none configured
    */
  ...
  /** Gets all available time manager types.
    * @return
    *   Set of available time manager type names
    */
