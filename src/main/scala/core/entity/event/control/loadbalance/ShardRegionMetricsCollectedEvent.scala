...
/** Internal self-message sent back to the LoadBalanceManager after a
  ...
  * message-processing loop: the Future records shard locations directly and then posts
  * this lightweight ack back to the LBM actor thread for logging.
  *
  * @param typeName   The shard region type name (e.g. "org.interscity.htc.model.Person")
  * @param shardCount The number of shards found in this region on the local node
  */
