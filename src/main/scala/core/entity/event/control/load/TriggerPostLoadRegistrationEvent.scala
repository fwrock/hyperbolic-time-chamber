...
/** Sent by LoadDataManager to PostLoadRegistrationCoordinator after all EAGER data sources
  * have finished loading. This triggers the coordinator to transition from accumulation phase
  * to execution phase: it fans out PostLoadRegistrationEvent to all entities that previously
