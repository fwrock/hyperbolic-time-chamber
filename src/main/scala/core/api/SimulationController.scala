...
/** Global controller for simulation lifecycle, accessible from API routes.
  *
  * Lifecycle events are forwarded to the appropriate Pekko singletons via
  * cluster singleton proxies. The controller tracks the high-level simulation
  * status and uses [[MetricsServer.currentTick]] to infer when loading
  * transitions to running.
  *
  * Status transitions:
  *   Idle  ──(start)──▶  Loading  ──(tick > 0)──▶  Running
  *                                                  │   ▲
  *                                               (pause)(resume)
  *                                                  ▼
  *                                               Paused
...
object SimulationController {
  ...
  /** Called once from [[ConfigApiServer]] right after the ActorSystem is created.
    *
    * @param apiEnabled  When true, [[SimulationManager]] will NOT auto-start;
    *                    the simulation waits for POST /api/v1/simulation/start.
    *                    When false, status is immediately set to Running to
    *                    reflect that the auto-start flow is in progress.
    */
  ...
  /** Current simulation status.
    * When in Loading state, transitions to Running automatically once the first
    * tick is recorded in Prometheus (avoids needing to modify SimulationManager).
    */
  ...
  /** Trigger simulation start. Applies any pending settings overrides, then
    ...
    *
    * @param configFile  Optional path to a simulation JSON file. When absent,
    *                    the configuration is resolved from [[ApiConfigRegistry]]
    *                    or the normal file/env-var chain.
    * @param settings    Optional htc.* settings overrides applied before start.
    */
  def start(
    ...
    withSystem { sys =>
      ...
      logger.info(
        s"Simulation start triggered via API" +
          configFile.filter(_.nonEmpty).map(f => s" (configFile=$f)").getOrElse("") +
  ...
  def pause(): Either[String, Unit] = {
    ...
    withSystem { sys =>
      DistributedUtil.createSingletonProxy(sys, GLOBAL_TIME_MANAGER_ACTOR_NAME) ! PauseSimulationEvent()
  ...
  def resume(): Either[String, Unit] = {
    ...
    withSystem { sys =>
      DistributedUtil.createSingletonProxy(sys, GLOBAL_TIME_MANAGER_ACTOR_NAME) ! ResumeSimulationEvent()
  ...
  def stop(): Either[String, Unit] = {
    ...
    withSystem { sys =>
      DistributedUtil.createSingletonProxy(sys, SIMULATION_MANAGER_ACTOR_NAME) ! StopSimulationEvent()
