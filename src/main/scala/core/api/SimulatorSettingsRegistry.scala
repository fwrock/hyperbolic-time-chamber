...
/** Thread-safe registry for htc.* setting overrides provided via the REST API.
  ...
  *   1. Values stored here (set via PUT /api/v1/settings)
  *   2. Environment variables (HTC_*)
  *   3. application.conf defaults
  *
  * When the API is NOT used, this registry is empty and has zero effect — all
  * configuration continues to come from application.conf / env vars as usual.
  *
  * Actors that want to honour API overrides should use the helper methods
  * [[getInt]], [[getString]], [[getBoolean]] instead of reading directly from
...
object SimulatorSettingsRegistry {
  ...
  val catalog: List[SettingDef] = List(
    ...
    SettingDef("htc.time-manager.max-instances-per-node", "HTC_TIME_MANAGER_PER_NODE",
      "Max LocalTM instances per node", "4"),
    SettingDef("htc.time-manager.lookahead-window",       "HTC_TIME_MANAGER_LOOKAHEAD",
      "Look-ahead window in ticks", "1"),
    SettingDef("htc.time-manager.window-size",            "HTC_TIME_MANAGER_WINDOW_SIZE",
      "Window size in ticks", "1"),
    SettingDef("htc.time-manager.metrics-log-interval",   "HTC_TIME_MANAGER_METRICS_INTERVAL",
      "Metrics log interval in ticks", "500"),
    SettingDef("htc.time-manager.actor-timeout-ms",       "HTC_TIME_MANAGER_ACTOR_TIMEOUT_MS",
      "Actor timeout in milliseconds", "180000"),
    SettingDef("htc.time-manager.sync-timeout-ms",        "HTC_TIME_MANAGER_SYNC_TIMEOUT_MS",
      "Sync timeout in milliseconds", "30000"),
    SettingDef("htc.time-manager.verbose-logging",        "HTC_TIME_MANAGER_VERBOSE_LOGGING",
    ...
    SettingDef("htc.report-manager.json.number-of-instances",       "HTC_REPORT_JSON_INSTANCES",
      "Total JSON reporter instances", "64"),
    SettingDef("htc.report-manager.json.number-of-instances-per-node", "HTC_REPORT_JSON_PER_NODE",
  ...
  def effectiveValue(key: String): String =
    Option(overrides.get(key))
      .orElse(catalogByPath.get(key).flatMap(d => sys.env.get(d.envVar)))
      .getOrElse {
  ...
  def effectiveSource(key: String): String =
    ...
    else catalogByPath.get(key).flatMap(d => sys.env.get(d.envVar)) match {
      case Some(_) => "env_var"
