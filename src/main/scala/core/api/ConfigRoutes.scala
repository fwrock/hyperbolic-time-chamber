...
/** Pekko HTTP routes for the optional simulator configuration REST API.
  *
  * Endpoints: GET /api/v1/config — returns the active configuration and its source PUT
  * /api/v1/config — loads a Simulation JSON as the API override DELETE /api/v1/config — clears the
  * API override (falls back to env/file) GET /api/v1/health — liveness check
  */
object ConfigRoutes {
              ...
              val (source, json) = ApiConfigRegistry.get match {
                ...
                case None =>
                  try {
                    ...
                    case e: Exception =>
                      logger.warn(
                        s"Config API GET: could not load config from file/env: ${e.getMessage}"
                      )
              ...
              entity(as[String]) {
                body =>
                  try {
                    ...
                    logger.info(
                      s"Config API: simulation '${simulation.name}' loaded via PUT /api/v1/config"
                    )
                    complete(
                      HttpResponse(
                        status = StatusCodes.OK,
