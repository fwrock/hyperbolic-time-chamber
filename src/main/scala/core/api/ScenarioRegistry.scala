...
/** Optional metadata file placed alongside simulation.json. Any field is optional — the file may
  * contain just a subset.
  *
...
/** Scans a configured directory for available simulation scenarios.
  ...
  *   1. `HTC_SCENARIOS_DIR` env var 2. `htc.api.scenarios-dir` in application.conf 3. Default:
  *      `/app/simulations`
  */
object ScenarioRegistry {
  ...
  def listScenarios(): List[ScenarioSummary] = {
    ...
    if (!dir.exists() || !dir.isDirectory) {
      logger.warn(
        s"ScenarioRegistry: directory '$scenariosDir' does not exist or is not a directory"
      )
      return List.empty
    }
    ...
    Option(dir.listFiles())
      ...
      .filter(
        f => f.isDirectory && new File(f, "simulation.json").exists()
      )
      .sortBy(_.getName)
      .map {
        scenarioDir =>
