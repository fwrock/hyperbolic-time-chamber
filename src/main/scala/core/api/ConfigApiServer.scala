...
/** Optional HTTP server that exposes the full simulator management REST API.
  *
  * Disabled by default — zero impact on the existing env-var / application.conf / JSON-file
  * configuration approach when not enabled.
  *
  * Enable: env: HTC_API_ENABLED=true (or htc.api.enabled = true in application.conf) port:
  * HTC_API_PORT=8080 (or htc.api.port = 8080, default 8080)
  *
  * When enabled, [[SimulationManager]] will NOT auto-start — the simulation waits for a POST
  * /api/v1/simulation/start call.
  *
  * Exposed endpoints: GET/PUT/DELETE /api/v1/simulation/config — scenario JSON GET
  * /api/v1/simulation/status — status + tick metrics POST /api/v1/simulation/start — start
  * (optional settings override) POST /api/v1/simulation/pause POST /api/v1/simulation/resume POST
  * /api/v1/simulation/stop GET/PUT/DELETE /api/v1/settings — htc.* settings catalog GET/PUT/DELETE
  * /api/v1/settings/{key} GET /api/v1/health
  */
object ConfigApiServer {
  ...
  def start(system: ActorSystem): Unit = {
    ...
    if (!apiEnabled) {
      logger.info(
        "Simulator API disabled (HTC_API_ENABLED not set). Running in env/file config mode."
      )
      return
    }
