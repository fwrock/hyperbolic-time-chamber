...
class CassandraReportData(
  ...
  private lazy val simulationId: String = {
    ...
    logInfo(s"📁 Source: ${
        if (simulationConfigId.isDefined) "simulation.json"
  ...
  private def initializeCassandra(): Unit =
    try {
      val sessionBuilder = CqlSession
        .builder()
        .addContactPoint(
          java.net.InetSocketAddress
            .createUnresolved(hosts.split(":")(0), hosts.split(":")(1).toInt)
        )
    ...
    session.foreach {
      cqlSession =>
        ...
        """
          ...
          try {
            cqlSession.execute(
              s"CREATE INDEX IF NOT EXISTS idx_report_type ON $keyspace.$table (report_type)"
            )
            cqlSession.execute(
              s"CREATE INDEX IF NOT EXISTS idx_simulation_id ON $keyspace.$table (simulation_id)"
            )
          } catch {
            ...
            report =>
              ...
              val dataJson =
                try
                  ...
                  case e: Exception =>
                    logError(
                      s"Failed to convert data to JSON, falling back to toString: ${e.getMessage}",
                      e
                    )
