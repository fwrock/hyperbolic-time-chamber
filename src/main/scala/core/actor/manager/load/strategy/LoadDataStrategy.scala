...
/** Base abstraction for data-loading strategies used by the load manager.
  *
  * A concrete implementation encapsulates how actor data is fetched and/or materialized from a
  * configured [[ActorDataSource]] and triggered by a [[LoadDataSourceEvent]].
  *
...
abstract class LoadDataStrategy(
  ...
  /** Executes the loading process using the full load request event.
    *
    * Use this entry point when the strategy needs access to orchestration references embedded in
    * the event (e.g., manager, creator, creator pool), in addition to the [[ActorDataSource]]
    * itself.
    *
    * @param event
    *   load request containing the target data source and runtime actor references required to
    *   coordinate creation/loading flow.
    */
  protected def load(event: LoadDataSourceEvent): Unit

  /** Executes the loading process using only a data source descriptor.
    *
    * Use this overload for source-focused loading where orchestration details are provided
    * elsewhere or not required by the strategy implementation.
    *
    * @param actorDataSource
    *   configuration that describes what to load (source id, class type, creation mode, backing
    *   source details, and loading strategy metadata).
    */
