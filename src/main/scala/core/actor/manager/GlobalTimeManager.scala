...
/** Global Time Manager - Central coordinator for distributed time management.
  *
  * The GlobalTimeManager serves as the master coordinator for all Local Time Managers in the
  * Hyperbolic Time Chamber simulation system. It manages the global simulation clock, coordinates
  * time advancement across different simulation paradigms, and creates distributed pools of Local
  * Time Managers using Apache Pekko clustering.
  *
  ...
  *
  * The GlobalTimeManager uses native Pekko ClusterRouterPool for efficient distribution of LTM
  * instances across cluster nodes, providing both scalability and fault tolerance.
  *
...
class GlobalTimeManager(
  ...
  /** Map of Local Time Manager pools organized by their type. Each entry contains a
    * ClusterRouterPool ActorRef for the specific LTM type.
    */
  private val localTimeManagerPools = mutable.Map[LocalTimeManagerTypeEnum, ActorRef]()

  /** Map of registered Local Time Managers with their coordination information. Contains ActorRef
    * to LocalTimeManagerInfo mappings for active LTMs.
    */
  ...
  /** Called when the Global Time Manager starts up.
    *
    * Performs parent class initialization and creates Local Time Manager pools for all supported
  ...
  /** Creates Props for a specific Local Time Manager type.
    *
    * This factory method creates the appropriate Props object for instantiating Local Time Managers
    * based on their type. Each LTM type has different constructor parameters and capabilities.
    *
    * @param ltmType
    *   The type of Local Time Manager to create Props for
    * @return
    *   Props object configured for the specific LTM implementation
    */
  ...
  /** Specific event receiver for Global Time Manager.
    ...
    *
    * @return
    *   Partial function for handling GTM-specific events
    */
  ...
  /** Registers a Local Time Manager with the Global Time Manager.
    *
    * Adds the LTM to the coordination registry and initializes its tracking information for time
    * synchronization.
    *
    * @param event
    *   The registration event containing LTM details
    */
  ...
  private def calculateAndBroadcastNextGlobalTick(): Unit = {
    ...
    localTimeManagers.keys.foreach {
      ltm =>
        localTimeManagers.get(ltm).foreach {
          info =>
  ...
  /** Returns statistics about the Global Time Manager.
    *
    * Provides comprehensive metrics about the global time coordination state including current
    * time, registered LTMs, active pools, and running status.
    *
    * @return
    *   Map containing key performance and state metrics
    */
...
object GlobalTimeManager {
  ...
  /** Creates Props for GlobalTimeManager actor instantiation.
    ...
    *   Reference to the main simulation manager
    * @return
    *   Props object for creating GlobalTimeManager actors
    */
  def props(
    simulationDuration: Tick,
    simulationManager: ActorRef
