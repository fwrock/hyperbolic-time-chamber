...
/** SnapshotManager
  *
  * Cluster-singleton manager that owns migration snapshot storage and acts as the
  * re-initialization authority after shard hand-off.
  *
  * === Protocol ===
  *
  * '''Save phase''' (entity being migrated, source node):
  *   1. LBM sends [[RegisterMigrationBatchEvent]] to register the batch (entityIds + lbmRef).
  *   2. Each entity sends [[SaveMigrationSnapshotEvent]] with its snapshot.
  *
  * '''Restore phase''' (entity recreated on target node):
  *   3. Entity sends [[QueryMigrationEvent]].
  *   4. SM replies with [[MigrationContextEvent]] if snapshot found,
  *      or [[NoPendingMigrationEvent]] if not (fresh entity, not migrated).
  *
  ...
  *   - default → In-Memory (SM-singleton-hosted, zero-overhead, works for all scenarios
  *     because the SM is a singleton accessible from any node via proxy)
  */
class SnapshotManager(timeManager: ActorRef)
  ...
  private def handleRegisterBatch(event: RegisterMigrationBatchEvent): Unit = {
    ...
    logInfo(s"SnapshotManager: registered batch '${event.batchId}' with ${event.entityIds.size} entities")
  }
  ...
  private def handleQueryMigration(event: QueryMigrationEvent): Unit = {
    pendingSnapshots.remove(event.entityId) match {
      case Some((snapshot, batchId)) =>
        ...
        logInfo(s"SnapshotManager: sending MigrationContextEvent to '${event.entityId}' (batch=$batchId)")
      ...
      case None =>
        logDebug(s"SnapshotManager: no pending snapshot for '${event.entityId}' — NoPendingMigrationEvent")
...
object SnapshotManager {
  ...
  /** Sent by SimulationManager once both poolTimeManagers and reporters are available.
    * This injects the migration initialization context (mirrors what CreatorLoadData does
    * via InitializeEvent during initial entity creation).
    */
