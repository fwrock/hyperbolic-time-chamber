...
/** Geographic Affinity balancing strategy.
  ...
  *
  *   2. '''Geographic affinity scoring''': When choosing the target pod for a dynamic shard
  *      migration the strategy scores each candidate using a weighted combination of compute load
  *      and geographic affinity:
  *      {{{
  *        score(pod) = (1 - α) * (1 / load(pod)) + α * affinity(pod, dynamicShardId)
  *      }}}
  *      where `α = geographicAffinityWeight` (default 0.4 — configurable in [[StrategyConfig]])
  *      and `affinity` rewards pods that already host:
  ...
  *
  * The underlying quadtree and kd-tree are the same components used by [[HybridStrategy]].
  * The difference is only in how shard IDs are assigned and how the migration target is chosen.
  */
class GeoAffinityStrategy extends BalancingStrategy {
  ...
  /** Assigns a prefixed shard ID: `"static-qt-shard-N"` or `"dynamic-qt-shard-N"`.
    *
    * The quadtree provides the base geographic shard ID; this method attaches the namespace
    * prefix so that static and dynamic entities in the same quadrant end up in different shards.
    */
  ...
  /** Evaluates migration need and selects targets using the affinity-weighted score.
    ...
    *   1. Predictive analysis (imminent overload of a dynamic shard)
    *   2. Periodic kd-tree rebalance (CPU imbalance ratio > 1.5 across pods)
    *
    * In both cases, the final target pod is chosen via [[selectTargetWithAffinity]] instead of
    * the plain min-load heuristic used by [[HybridStrategy]].
    *
  ...
  override def evaluate(): List[MigrationPlan] = {
    ...
    migrations.filter(p => isMigratable(p.shardId)).toList.sortBy(-_.priority)
  }
