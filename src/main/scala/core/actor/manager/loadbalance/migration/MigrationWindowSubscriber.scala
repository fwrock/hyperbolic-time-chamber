...
import core.entity.event.control.migration.{ MigrationWindowAckEvent, MigrationWindowCloseEvent, MigrationWindowOpenEvent }
...
/** Per-node actor that participates in the distributed migration window protocol.
  *
  * One instance is started by [[core.HyperbolicTimeChamber]] on every cluster node (regular actor,
  * NOT a singleton). It:
  *   1. Creates a cluster-singleton proxy for the SnapshotManager and registers it in
  *      [[MigrationStateStoreRegistry]] so that entities on this JVM can reach SM without a
  *      blocking lookup. 2. Subscribes to the DistributedPubSub topic `"migration-window"`. 3. When
  *      [[MigrationWindowOpenEvent]] arrives from the LBM:
  *      - Sets [[MigrationStateStoreRegistry.isMigrationActive]] = true
  *      - ACKs to event.lbmRef with phase = "open" 4. When [[MigrationWindowCloseEvent]] arrives
  *        from the LBM:
  *      - Sets [[MigrationStateStoreRegistry.isMigrationActive]] = false
  *      - ACKs to event.lbmRef with phase = "close"
  *
  * ===Why per-node (not singleton)?===
  * The flag must be set on *every* JVM before Pekko hands off shards, so the subscriber needs to
  * run locally on each node and update the JVM-local AtomicBoolean directly. A singleton would only
  * run on one node and defeat the purpose.
  */
class MigrationWindowSubscriber extends Actor with ActorLogging {
  ...
  override def receive: Receive = {
    case SubscribeAck(Subscribe(MigrationWindowSubscriber.TOPIC, _, _)) =>
      log.info(
        "MigrationWindowSubscriber: subscribed to topic '{}'",
        MigrationWindowSubscriber.TOPIC
      )

    case event: MigrationWindowOpenEvent =>
      log.info(
        "MigrationWindowSubscriber [{}]: window OPEN for batch '{}'",
        nodeAddr,
        event.batchId
      )
    ...
    case event: MigrationWindowCloseEvent =>
      log.info(
        "MigrationWindowSubscriber [{}]: window CLOSE for batch '{}'",
        nodeAddr,
        event.batchId
      )
