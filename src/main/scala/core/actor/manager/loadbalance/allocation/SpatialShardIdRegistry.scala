...
/** Thread-safe global registry mapping entity IDs to spatially-assigned shard IDs.
  *
  * This registry decouples the spatial assignment (performed by [[LoadBalanceManager]])
  * from the shard routing (performed by Pekko's `extractShardId` in [[core.util.ActorCreatorUtil]]).
  *
  * Flow:
  *   1. During entity creation, [[core.actor.manager.load.CreatorLoadData]] sends batch
  *      spatial data to [[core.actor.manager.loadbalance.LoadBalanceManager]].
  *   2. LoadBalanceManager assigns shard IDs via the Quadtree/strategy.
  *   3. Assignments are stored here by CreatorLoadData when it receives the response.
  *   4. When Pekko routes messages, `extractShardId` in ActorCreatorUtil checks this
  *      registry first; if found, uses the spatial shard ID instead of hash-based.
  *
...
object SpatialShardIdRegistry {
  ...
  /** Entity ID → position (x/lon, y/lat). Used by creation pipeline to resolve
  ...
  /** Stores an entity's spatial position for later lookup.
    * Primarily used for nodes so that links/cars can resolve positions via node IDs.
    */
  ...
  /** Returns all entity IDs assigned to a given shard.
    ...
    * notifications but not for hot-path operations.
    */
  def getEntitiesInShard(shardId: String): Set[String] = {
    import scala.jdk.CollectionConverters._
    shardAssignments.entrySet().asScala
