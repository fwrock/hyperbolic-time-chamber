...
/** Mixin trait for time managers that need to coordinate microscopic simulation.
  ...
  * 
  * Architecture:
  * 1. Micro links register themselves with LocalTimeManager
  * 2. At each global tick, LocalTimeManager triggers all micro links
  * 3. Each link executes its sub-ticks locally via LinkMicroTimeManager
  * 4. Links report completion back to LocalTimeManager
  * 5. LocalTimeManager advances to next tick after all links complete
  * 
  * This design provides spatial locality (vehicles on same link stay together)
  * and reduces message passing to global time manager.
  */
trait MicroAwareTimeManager extends TimeManagerBase {
  
  /** Set of micro-enabled link actor references.
    * These links run microscopic simulation with sub-ticks.
    */
  private val microLinks = scala.collection.mutable.Set[ActorRef]()
  
  /** Register a link as micro-enabled.
    * 
    * @param linkRef The actor reference of the micro link
    */
  ...
  /** Trigger all micro links to execute sub-ticks for the current global tick.
    * 
    * @param tick The current global tick
    */
  ...
  /** Get the number of registered micro links.
    * 
    * @return The count of micro-enabled links
    */
  def getMicroLinkCount: Int = microLinks.size
  
  /** Check if there are any micro links registered.
    * 
    * @return true if at least one micro link is registered
    */
