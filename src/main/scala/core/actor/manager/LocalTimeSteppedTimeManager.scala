...
/** Local Time Manager for Time-Stepped Simulation. This manager advances time at fixed intervals
  * (time steps), processing all actors at each step regardless of whether they have scheduled
  * events. This approach is useful for continuous simulations where actors need to update state at
  * regular intervals.
  *
  ...
  * @param timeStep
  *   The fixed time step size (default: 1 tick)
  */
class LocalTimeSteppedTimeManager(
  ...
  /** Processes a tick in time-stepped manner. At each time step, all registered actors are
    * triggered, regardless of whether they have scheduled events. Time advances in fixed
    * increments.
    *
    * @param tick
    *   The tick to process
  ...
  protected def processTick(tick: Tick): Unit = {
    if (tick % 10000 == 0) {
      logInfo(
        s"[LocalTimeStepped] Processing tick $tick with ${registeredActors.size} registered actors"
      )
    }
  ...
  private def triggerAllActors(tick: Tick): Unit = {
    ...
    if (scheduledAtThisTick.nonEmpty) {
      if (tick % 1000 == 0) {
        logInfo(
          s"[LocalTimeStepped] Triggering ${scheduledAtThisTick.size} scheduled actors at tick $tick"
        )
      }
