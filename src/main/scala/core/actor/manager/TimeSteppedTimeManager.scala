...
import core.entity.event.control.simulation.{ AdvanceToTick, BarrierSynchronization, TickCompleted }
...
/** Time-Stepped Simulation Time Manager implementation.
  *
  * This Local Time Manager implements the Time-Stepped simulation paradigm, providing synchronized
  * time advancement where all actors proceed in lockstep through discrete time intervals (steps).
  *
  ...
  *
  * The Time-Stepped approach ensures that all actors complete their processing for the current time
  * step before any actor can advance to the next step, providing strong consistency guarantees.
  *
  ...
  * @param stepSize
  *   The size of each time step in simulation ticks (default: 1)
  *
  * @author
...
class TimeSteppedTimeManager(
  ...
  override protected def handleSpecificEvent(event: Any): Unit = event match {
    ...
    case _ =>
      logDebug(s"Time-Stepped: Evento específico não tratado: ${event.getClass.getSimpleName}")
  }
  ...
  private def broadcastAdvanceToAllActors(targetTick: Tick): Unit = {
    logInfo(
      s"Time-Stepped: Broadcasting AdvanceToTick($targetTick) para ${registeredActors.size} atores"
    )
...
object TimeSteppedTimeManager {
  def props(
    ...
    Props(
      classOf[TimeSteppedTimeManager],
      simulationDuration,
      simulationManager,
      parentManager,
      stepSize
