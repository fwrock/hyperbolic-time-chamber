...
import core.entity.event.control.simulation.{ OptimisticEvent, WindowEnd, WindowStart }
...
/** Optimistic Time Window Time Manager implementation.
  *
  * This Local Time Manager is designed for optimistic simulation strategies that allow speculative
  * execution and rollback capabilities. Currently implemented as a DES-compatible placeholder while
  * the full optimistic algorithms are being developed.
  *
  ...
  * @param windowSize
  *   Size of the optimistic time window (currently unused)
  *
  * @author
...
class OptimisticTimeWindowTimeManager(
  ...
  override protected def onSimulationStart(start: StartSimulationTimeEvent): Unit = {
    ...
    logInfo(
      s"Time Window Time Manager iniciado no tempo ${start.startTick} (windowSize: $windowSize)"
    )
  ...
  private def handleWindowStart(windowStart: WindowStart): Unit = {
    logInfo(
      s"Time Window: Iniciando janela no tick ${windowStart.startTick} com tamanho ${windowStart.windowSize}"
    )
  ...
  private def cleanupOptimisticState(): Unit = {
    ...
    if (rollbackPoints.size > 1) {
      ...
      rollbackPoints.retain(
        (tick, _) => tick == latestTick
      )
    }
...
object OptimisticTimeWindowTimeManager {
  def props(
    ...
    Props(
      classOf[OptimisticTimeWindowTimeManager],
      simulationDuration,
      simulationManager,
      parentManager,
      windowSize
