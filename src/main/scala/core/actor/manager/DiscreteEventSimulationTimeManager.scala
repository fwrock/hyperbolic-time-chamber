...
/** Discrete Event Simulation Time Manager implementation.
  *
  * This Local Time Manager implements the Discrete Event Simulation (DES) paradigm, providing
  * event-driven time management where simulation time advances based on the occurrence of discrete
  * events rather than fixed time steps.
  *
  ...
  *
  * This implementation preserves the original DES behavior while integrating with the new
  * object-oriented time management architecture. It accepts event scheduling and processes events
...
class DiscreteEventSimulationTimeManager(
  ...
  /** Indicates that DES accepts event scheduling (original DES behavior).
    *
    * DES is event-driven and relies on scheduling events at specific simulation times for proper
    * operation.
    *
    * @return
    *   true, as DES always accepts event scheduling
    */
  override protected def acceptsScheduling: Boolean = true

  /** Creates a spontaneous event for an actor (original DES behavior).
    *
    * In DES, actors are triggered by spontaneous events that carry the simulation time and manager
    * reference.
    *
    * @param tick
    ...
    * @return
    *   A spontaneous event configured for DES operation
    */
...
object DiscreteEventSimulationTimeManager {
  ...
  /** Creates Props for DiscreteEventSimulationTimeManager actor instantiation.
    ...
    *   Reference to the Global Time Manager for coordination
    * @return
    *   Props object for creating DES Time Manager actors
    */
  def props(
    simulationDuration: Tick,
    simulationManager: ActorRef,
    ...
    Props(
      classOf[DiscreteEventSimulationTimeManager],
      simulationDuration,
      simulationManager,
      parentManager
    )
