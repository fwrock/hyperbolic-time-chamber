    ...
    triedClazz match {
      case Success(clazz) =>
        Try {
          ...
          val parseFromMethod =
            companionObject.getClass.getMethod("parseFrom", classOf[Array[Byte]])
          ...
          case Failure(e) =>
            val ex = new NotSerializableException(
              s"Falha ao invocar parseFrom para o manifesto [$manifest]"
            )
            ex.initCause(e)
            throw ex
          case Success(other) =>
            throw new NotSerializableException(
              s"parseFrom retornou um tipo inesperado [$other] para o manifesto [$manifest]"
            )
      ...
      case Failure(exception) =>
        throw {
          val ex = new NotSerializableException(
            s"Não foi possível carregar a classe para o manifesto [$manifest]: ${exception.getMessage}"
          )
