...
import org.interscity.htc.core.actor.manager.report.{ ClickHouseReportData, CsvReportData, JsonReportData, KafkaReportData, ParquetReportData, ReportData }
...
enum ReportTypeEnum(val clazz: Class[? <: ReportData]) {
  ...
  case kafka extends ReportTypeEnum(classOf[KafkaReportData])
}
