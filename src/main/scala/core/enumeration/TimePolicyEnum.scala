 ...
 * 
 * Define qual LocalTimeManager (LTM) será responsável por gerenciar o tempo
 * dos atores registrados com cada política.
 */
object TimePolicyEnum extends Enumeration {
  ...
  def fromString(str: String): TimePolicyEnum = {
    str.toLowerCase match {
      ...
      case "timestepped" | "time-stepped" | "stepped" | "timesteppedsimulation" => TimeSteppedSimulation
      case "timewindow" | "time-window" | "window" | "optimistic" | "optimisticsimulation" => OptimisticSimulation
  ...
  def getDescription(policy: TimePolicyEnum): String = {
    policy match {
      case DiscreteEventSimulation => "Discrete Event Simulation - eventos processados em ordem cronológica"
      case TimeSteppedSimulation => "Time-Stepped Simulation - passos de tempo sincronizados entre atores"
      case OptimisticSimulation => "Optimistic Simulation - simulação otimística com janelas de tempo"
