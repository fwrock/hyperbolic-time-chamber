...
class CassandraEntityManager(sessionName: String, keyspace: String)
    extends Actor
    with ActorLogging {
  ...
  val session: CassandraSession =
    CassandraSessionRegistry.get(context.system).sessionFor(sessionName)

  private var preparedStatementCache =
    collection.mutable.Map.empty[String, Future[PreparedStatement]]
  ...
  override def receive: Receive = {
    ...
    case event: ReadEntityEvent =>
      ...
      getOrPrepare(cql).flatMap {
        ...
        rows => QuerySuccess(rows.map(rowToMap))
      ).pipeTo(senderRef)

    case event: UpdateEntityEvent =>
      val senderRef = sender()
      val setClause = event.setColumns.keys
        .map(
          col => s"$col = ?"
        )
        .mkString(", ")
  ...
  /** Obtém um PreparedStatement do cache ou o prepara se for a primeira vez. Isso evita preparar a
    * mesma query repetidamente.
    */
  private def getOrPrepare(cql: String): Future[PreparedStatement] =
    preparedStatementCache.getOrElseUpdate(
      cql, {
  ...
  /** Converte um objeto Row do driver do Cassandra para um Map[String, AnyRef] genérico. Isso torna
    * o resultado mais fácil de usar pelo ator que fez a requisição.
    */
