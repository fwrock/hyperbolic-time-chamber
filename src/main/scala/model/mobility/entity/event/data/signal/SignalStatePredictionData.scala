...
/** Event-driven: Time-bounded signal state prediction
  *
  * Allows vehicles to plan travel without synchronous queries. Provides deterministic, time-bounded
  * signal state information.
  *
  * @param currentState
  *   Current phase state
  * @param validUntil
  *   Tick until current state is guaranteed
  * @param nextState
  *   State after validUntil (if known)
  * @param nextTransitionTick
  *   Tick of next transition (if within horizon)
  */
