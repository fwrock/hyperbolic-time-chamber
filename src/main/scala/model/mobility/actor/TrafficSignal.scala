...
class TrafficSignal(
  ...
  private def scheduleAllPhaseTransitions(): Unit = {
    ...
    logDebug(
      s"Pre-scheduling phase transitions for signal ${getEntityId} until tick $simulationEnd"
    )
    ...
    state.signalStates.get(phaseChange.phaseOrigin).foreach {
      signalState =>
        ...
        if (oldState != phaseChange.newState) {
          TrafficSignalMetrics.phaseChanges.labels(phaseChange.newState.toString).inc()
          logDebug(
  ...
  def getSignalStatePrediction(
    ...
    val currentState = state.signalStates
      .get(phaseOrigin)
      .map(_.state)
      .getOrElse(Red)
