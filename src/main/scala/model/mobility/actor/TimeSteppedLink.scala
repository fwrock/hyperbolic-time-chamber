...
import model.mobility.entity.state.micro.{MicroVehicleContext, MicroVehicleIntention, MicroVehicleState}
import model.mobility.entity.event.micro.{MyMicroIntention, ProvideMicroContext}
...
class TimeSteppedLink(
  ...
  override def onInitialize(event: InitializeEvent): Unit = {
    ...
    logInfo(
      s"TimeSteppedLink ${getEntityId} inicializado com política TimeStepped, tipo: ${state.simulationType}"
    )
  }
  ...
  /** TimeStepped behavior - processes all queued events at each tick Supports both mesoscopic and
    * microscopic simulation
    */
  override def actAdvanceToTick(event: AdvanceToTick): Unit = {
    val targetTick = event.targetTick
    logDebug(
      s"TimeSteppedLink ${getEntityId} processando tick $targetTick (${state.simulationType})"
    )
  ...
  private def buildMicroContext(vehicle: MicroVehicleState): MicroVehicleContext = {
    ...
    MicroVehicleContext(
      ...
      leftLeader =
        if (vehicle.lane > 0) TrafficModels.findLeader(vehicle.position, vehicle.lane - 1, vehicles)
        else None,
      leftFollower =
        if (vehicle.lane > 0)
          TrafficModels.findFollower(vehicle.position, vehicle.lane - 1, vehicles)
        else None,
      rightLeader =
        if (vehicle.lane < state.lanes - 1)
          TrafficModels.findLeader(vehicle.position, vehicle.lane + 1, vehicles)
        else None,
      rightFollower =
        if (vehicle.lane < state.lanes - 1)
          TrafficModels.findFollower(vehicle.position, vehicle.lane + 1, vehicles)
  ...
  private def waitForMicroIntentions(): Unit = {
    ...
    if (microIntentions.size < expectedIntentions) {
      logWarn(
        s"Nem todas as intenções foram recebidas: ${microIntentions.size}/$expectedIntentions"
      )
    }
  }
  ...
  private def resolveLaneChangeConflicts(
    ...
    for ((lane, candidates) <- conflicts)
      if (candidates.size == 1) {
        ...
        val vehicleStates = candidates.map(
          id => (id, state.microVehicles(id))
        )
  ...
  private def processEnterLink(data: EnterLinkData): Unit =
    if (state != null) {
      ...
      logDebug(
        s"Veículo ${data.actorId} entrou no link ${getEntityId}. Total veículos: ${state.registered.size}"
      )
    }
  ...
  private def createMicroVehicleState(data: EnterLinkData): Unit = {
    ...
    val laneOccupancy = (0 until state.lanes).map {
      lane =>
  ...
  private def processLeaveLink(data: LeaveLinkData): Unit =
    if (state != null) {
      ...
      if (beforeSize != afterSize) {
        logDebug(
          s"Veículo ${data.actorId} saiu do link ${getEntityId}. Total veículos: ${afterSize}"
        )
      } else {
        logWarn(
          s"Tentativa de remover veículo ${data.actorId} que não estava no link ${getEntityId}"
        )
      }
    }
  ...
  private def updateLinkState(tick: Long): Unit = {
    ...
    logDebug(
      s"Link ${getEntityId} tick $tick: ${state.registered.size} veículos, " +
  ...
  def getLinkStatistics: Map[String, Any] = {
    ...
    if (state.simulationType == "micro" && state.microVehicles.nonEmpty) {
      val microStats = Map(
        ...
        "avgPosition" -> (state.microVehicles.values
          .map(_.position)
          .sum / state.microVehicles.size),
