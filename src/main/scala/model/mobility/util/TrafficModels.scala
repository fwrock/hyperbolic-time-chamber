...
object TrafficModels {
   ...
   * 
   * @param vehicle Estado do veículo atual
   * @param leader Estado do veículo líder (opcional)
   * @return Aceleração calculada em m/s²
   */
  def calculateIDMAcceleration(
    ...
    leader match {
      case Some(lead) =>
        val s = math.max(0.1, lead.position - vehicle.position - 5.0) // gap real (assumindo 5m de comprimento do veículo)
   ...
   * 
   * @param vehicle Estado do veículo que deseja trocar de faixa
   * @param targetLane Faixa de destino
   * @param currentContext Contexto atual do veículo
   * @param targetLeader Líder na faixa de destino
   * @param targetFollower Seguidor na faixa de destino
   * @return True se a troca de faixa deve ser realizada
   */
   ...
   * 
   * @param vehicle Estado atual do veículo
   * @param acceleration Aceleração a ser aplicada
   * @param timestep Intervalo de tempo
   * @return Novo estado do veículo
   */
  def updateVehicleKinematics(
    ...
    val limitedAccel = math.max(-vehicle.maxSafeDeceleration, 
                               math.min(vehicle.maxAcceleration, acceleration))
   ...
   * 
   * @param vehiclePosition Posição do veículo de referência
   * @param lane Faixa a ser analisada
   * @param vehicles Lista de todos os veículos no link
   * @return Veículo líder mais próximo (opcional)
   */
  def findLeader(
    ...
    vehicles
      .filter(v => v.lane == lane && v.position > vehiclePosition)
      .toSeq
      .sortBy(_.position)
      .headOption
  }
   ...
   * @param lane Faixa a ser analisada
   * @param vehicles Lista de todos os veículos no link
   * @return Veículo seguidor mais próximo (opcional)
   */
  def findFollower(
    vehiclePosition: Double,
    lane: Int,
   ...
   * 
   * @param vehicle Veículo que deseja trocar
   * @param targetLeader Líder na faixa de destino
   * @param targetFollower Seguidor na faixa de destino
