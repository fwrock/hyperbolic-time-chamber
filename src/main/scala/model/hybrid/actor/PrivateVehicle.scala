...
/** Common trait for private vehicles (Car, Bicycle, Motorcycle).
  ...
  * 
  * This trait provides common behavior for activation/deactivation
  * and driver attribute configuration.
  */
trait PrivateVehicle[T <: MovableState] {
  ...
  /** Accessor methods for protected BaseActor/SimulationBaseActor members.
  ...
  protected def sendVehicleMessage(entityId: String, shardId: String, data: AnyRef, eventType: String, actorType: CreationTypeEnum): Unit
  ...
  /** Initialize vehicle in Parked state.
    * 
    * Override actSpontaneous in subclasses to call this.
    * Vehicles should NOT auto-start; they wait for StartTrip message.
    */
  ...
  /** Handle StartTrip message from Person.
    * 
    * Activates the vehicle, configures it with driver attributes,
    * and begins the trip.
    */
  ...
  /** Apply driver attributes to vehicle physics parameters.
    * 
    * Subclasses should override this to apply attributes to their
    * specific state (CarState, BicycleState, etc.).
    */
  protected def applyDriverAttributes(attrs: DriverAttributes): Unit = {
    logVehicleDebug(s"Applying driver attributes: aggressiveness=${attrs.aggressiveness}, " +
