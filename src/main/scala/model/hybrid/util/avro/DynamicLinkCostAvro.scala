 ...
 * 
 * Temporarily using JSON instead of binary Avro until plugin issues are resolved.
 * Converts between internal DynamicLinkCost model and DynamicLinkCostEvent
 * for efficient Kafka serialization.
 */
object DynamicLinkCostAvro {
  ...
  def toAvro(cost: DynamicLinkCost): DynamicLinkCostEvent = {
    DynamicLinkCostEvent.newBuilder()
      ...
      .setNodeId(try { java.net.InetAddress.getLocalHost.getHostName } catch { case _ => "unknown" })
