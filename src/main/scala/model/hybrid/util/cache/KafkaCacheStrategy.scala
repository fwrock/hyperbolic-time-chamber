...
/** Kafka-based cache strategy for dynamic link costs.
  * 
  * Architecture:
  * 1. LinkActors publish cost updates to Kafka topic "dynamic-link-costs"
  * 2. Each cluster node consumes from topic asynchronously
  * 3. Local ConcurrentHashMap updated with latest costs
  * 4. A* algorithm uses ultra-fast local memory access
  * 5. Accepts eventual consistency (realistic like real traffic)
  * 
  * Benefits:
...
class KafkaCacheStrategy(implicit ec: ExecutionContext) extends WeightCacheStrategy {
  ...
  // Use standardized topic naming
  private val TOPIC_NAME = KafkaTopicNaming.Routing.DynamicCosts
  
  // Kafka producer for publishing costs
  private lazy val producer: KafkaProducer[String, String] = {
    ...
    props.put(ProducerConfig.ACKS_CONFIG, "1") // Leader ack (balance speed/reliability)
    props.put(ProducerConfig.RETRIES_CONFIG, "3")
    props.put(ProducerConfig.LINGER_MS_CONFIG, "5") // Batch for efficiency
  ...
  private lazy val consumer: KafkaConsumer[String, String] = {
    ...
    // Generate a safe consumer group ID using container/cluster info
    val consumerGroupId = try {
      // Try to get hostname, fallback to container name or random ID if it fails
      val hostname = sys.env.getOrElse("CLUSTER_NAME", 
    ...
    props.put(ConsumerConfig.MAX_POLL_RECORDS_CONFIG, "500") // Batch processing
  ...
  private def kafkaBootstrapServers: String = {
    try {
      ...
      case _: Exception => "localhost:9092" // Default
    }
  }
  ...
  override def publishCost(cost: DynamicLinkCost, ttlSeconds: Int): Try[Unit] = {
    Try {
      ...
      // Async send for performance
      producer.send(record, (metadata, exception) => {
        if (exception != null) {
          System.err.println(s"Failed to publish cost for link ${cost.linkId}: ${exception.getMessage}")
        } else {
          publishCount += 1
        }
      })
      
      // Also update local cache immediately for this node
  ...
  override def getWeight(linkId: String, staticWeight: Double): Double = {
    getCost(linkId) match {
      ...
      case None => staticWeight // Fallback to static weight
    }
  }
  
  override def getBatchWeights(linkWeights: Map[String, Double]): Map[String, Double] = {
    // Ultra-fast batch processing using local cache
  ...
  override def clearCost(linkId: String): Try[Unit] = {
    Try {
      ...
      // Note: We don't publish "delete" events to Kafka for simplicity
      // Costs will naturally expire based on timestamp age
    }
  }
