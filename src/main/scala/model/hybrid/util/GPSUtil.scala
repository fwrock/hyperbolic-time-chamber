...
object GPSUtil {
  ...
  /** Calcula a rota usando Contraction Hierarchies com A* (CH+A*).
    *
    * Combina a poda hierárquica do CH com a heurística do A* (distância euclidiana),
    * resultando na busca mais rápida entre os algoritmos disponíveis para grafos estáticos.
    *
    ...
    * @return Option contendo (custo estático, fila de rota) ou None se a rota não for encontrada.
    */
    ...
    (originNodeOpt, destinationNodeOpt) match {
      case (Some(originNode), Some(destinationNode)) =>
        CityMapUtil.chIndex.queryAStar(originNode, destinationNode, heuristicFunc) match {
          case Some((cost, path)) =>
            val routeQueue = mutable.Queue[(String, String)]()
            path.foreach { case (edgeLabel, targetNode) =>
  ...
  /** Calcula a rota usando CH+A* com índice reconstruído de forma adaptativa.
    ...
    * [[calcRouteCHAStar]]. Pesos dinâmicos do Kafka só influenciam ''quando'' reconstruir,
    * não os pesos internos do CH após a reconstrução.
    *
    * Para tráfego contínuo em tempo real use [[calcRoute]] com `useDynamicWeights = true`.
    *
    * @param originId      ID do nó de origem.
    * @param destinationId ID do nó de destino.
    * @param currentTick   Tick global atual da simulação.
    * @param policy        Política de reconstrução — padrão [[CHRebuildPolicy.fromConfig]].
  ...
  def calcRouteCHAStarAdaptive(
    ...
    (originNodeOpt, destinationNodeOpt) match {
      ...
      case (None, _) =>
        System.err.println(s"GPSUtil (CH+A* adaptive): Nó de origem $originId não encontrado no mapa.")
        None
      case (_, None) =>
        System.err.println(s"GPSUtil (CH+A* adaptive): Nó de destino $destinationId não encontrado no mapa.")
        None
    }
  }
}
