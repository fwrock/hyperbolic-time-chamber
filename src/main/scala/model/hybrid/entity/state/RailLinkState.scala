...
/** State for rail link (dedicated for subway/train transit).
  ...
  * network from road links. Cars, buses, and other road vehicles
  * CANNOT use rail links.
  *
  ...
  * @param lanes Number of tracks (typically 2 for bidirectional)
  * @param speedLimit Maximum speed in km/h
  * @param capacity Maximum trains per hour
  * @param freeSpeed Free-flow speed in km/h
  * @param railType Type of rail (SUBWAY, LIGHT_RAIL, HEAVY_RAIL)
  * @param subwayLine Subway line ID this rail belongs to
  * @param fromStation Origin station ID
  * @param toStation Destination station ID
  * @param gradient Track gradient (positive = uphill, negative = downhill)
  * @param curvature Track curvature (higher = more curved, affects speed)
  * @param simulationMode Simulation mode (typically MESO for rails)
  * @param registered Registered vehicles
  * @param scheduleOnTimeManager Schedule on time manager
  */
case class RailLinkState(
  ...
  /** Calculate effective speed considering gradient and curvature.
    ...
    * - Sharp curves
    *
    * @return Effective speed in km/h
    */
  ...
  /** Check if a vehicle type can use this rail link.
    ...
    *
    * @param vehicleType Vehicle type (e.g., "Subway", "Car")
    * @return True if vehicle can use this rail
    */
