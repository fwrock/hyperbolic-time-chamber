...
/** Base trait for microscopic state of all movable entities (vehicles, bicycles, etc.).
  *
  * This trait defines the common interface for microscopic simulation state. All vehicles in MICRO
  * mode must implement this interface to work with car-following models, lane-change models, and
  * micro time managers.
  *
