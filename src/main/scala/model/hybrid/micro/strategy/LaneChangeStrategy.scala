...
/** Strategy interface for lane-change decision making in microscopic simulation.
  *
  * Implementations define when and how vehicles should change lanes, considering factors like gap
  * acceptance, incentive to change lanes (MOBIL model), and safety constraints.
  *
  * This interface is provided for future extensibility. Currently, the system does not implement
  * active lane changing during simulation.
  *
  * @see
  *   model.hybrid.micro.lane.MobilLaneChange for future MOBIL implementation
  */
trait LaneChangeStrategy {
  ...
  /** Determines if a vehicle should change lanes and to which lane.
    *
    * @param vehicleId
    *   ID of the vehicle considering lane change
    * @param currentLane
    ...
    *   Speed limit in km/h
    * @return
    *   Optional target lane ID if lane change is desired, None otherwise
    */
  ...
  /** Checks if a lane change is safe (sufficient gap in target lane).
    *
    * @param vehicle
    *   Vehicle attempting lane change
    * @param targetLane
    *   Target lane ID
    * @param vehiclesByLane
    *   State of all lanes
