...
/** Strategy interface for microscopic traffic simulation algorithms.
  *
  * Implementations of this interface define how vehicles move and interact within a microscopic
  * link during sub-tick execution. Different strategies can implement various car-following models,
  * lane-change behaviors, and traffic flow dynamics.
  *
  * @see
  *   DefaultMicroSimulationStrategy for the default Krauss-based implementation
  */
trait MicroSimulationStrategy {
  ...
  /** Executes a single sub-tick of microscopic simulation for all lanes.
    ...
    *   Map of lane ID to queue of vehicles in that lane
    * @param subTick
    *   Current sub-tick index (0 to microTicksPerGlobalTick-1)
    * @param tick
    *   Current global tick
    * @param linkLength
    *   Total length of the link in meters
    * @param speedLimit
    *   Speed limit of the link in km/h
    * @param microTimeStep
    *   Duration of each sub-tick in seconds
    * @param microTicksPerGlobalTick
    *   Number of sub-ticks per global tick
    * @param vehicleWaitingSeconds
    *   Map tracking accumulated waiting time per vehicle
    * @return
    *   Sequence of updates to send to vehicles
    */
  def executeSubTick(
  ...
  /** Determines the best lane for a vehicle entering the link.
    *
    * @param vehiclesByLane
    *   Current state of all lanes
    * @param vehicleId
    *   ID of the entering vehicle
    * @param vehicleLength
    *   Length of the vehicle in meters
    * @return
    *   Lane ID (0-indexed) for the vehicle to enter
    */
  def selectEntryLane(
    vehiclesByLane: mutable.Map[Int, mutable.Queue[VehicleInLane]],
    vehicleId: String,
...
/** Container for micro-simulation update data to be sent to a vehicle.
  ...
  * @param shardId
  *   Shard ID where the vehicle actor resides
  * @param subTick
  *   Sub-tick index when this update was generated
  * @param position
  *   Current position in the link (meters from start)
  * @param velocity
  ...
  *   Current lane ID
  * @param leaderVehicle
  *   Optional ID of the vehicle ahead
  * @param gapToLeader
  *   Gap to leader vehicle in meters
  * @param leaderVelocity
  *   Leader vehicle velocity in m/s
  * @param safeVelocity
