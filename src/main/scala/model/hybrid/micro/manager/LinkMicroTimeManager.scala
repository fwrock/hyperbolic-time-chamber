...
/** LinkMicroTimeManager actor.
  *
  * Manages sub-tick execution for microscopic simulation within a single link. Acts as a local time
  * manager to avoid bottlenecks in the global TimeManager.
  *
  * Responsibilities:
...
class LinkMicroTimeManager(
  ...
  /** Internal state: vehicles organized by lane. Key: lane ID Value: Queue of vehicles (sorted by
    * position, front to back)
    */
  private val vehiclesByLane: mutable.Map[Int, mutable.Queue[VehicleInLane]] =
    mutable.Map(
      (0 until numberOfLanes).map(
        lane => lane -> mutable.Queue.empty[VehicleInLane]
      ): _*
    )
  ...
  private def registerVehicle(
    ...
    context.log.debug(
      s"[$linkId] Registering vehicle $vehicleId in lane $lane at position $position"
    )
  ...
  private def updateVehicleState(
    ...
    vehiclesByLane.get(lane).foreach {
      queue =>
        ...
        fromQueue.find(_.actorId == vehicleId).map {
          vehicle =>
            if (toLane >= 0 && toLane < numberOfLanes) {
              ...
              context.log
                .info(s"[$linkId] Vehicle $vehicleId changed from lane $fromLane to $toLane")
            } else {
...
object LinkMicroTimeManager {
  ...
  def apply(
    ...
    context =>
      context.log.info(
        s"[$linkId] LinkMicroTimeManager started: $numberOfLanes lanes, ${linkLength}m, ${microTimeStep}s steps"
      )
