...
class KafkaAbstractionSpec extends AnyFlatSpec with Matchers with ScalaFutures {
  ...
  "KafkaAbstractionFactory.Consumers" should "create specialized consumers" in {
    ...
    try {
      ...
      val reportConsumer =
        KafkaAbstractionFactory.Consumers.forReports[TestMessage]("test-topic", "test-group")
  ...
  "KafkaAbstractionFactory.Brokers" should "create specialized brokers" in {
    ...
    try {
      val requestResponseBroker =
        KafkaAbstractionFactory.Brokers.forRequestResponse[TestMessage, TestMessage](
      ...
      val commandEventBroker =
        KafkaAbstractionFactory.Brokers.forCommandEvent[TestMessage, TestMessage](
