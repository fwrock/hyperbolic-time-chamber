...
class AntiMessageCascadeSpec extends AnyFlatSpec with Matchers {
  ...
  private def sentMessage(senderId: String, tick: Tick, seq: Long, receiverId: Long): SentMessage =
    SentMessage(
  ...
  it should "return every sent message across every undone event, unconditionally" in {
    val m1 = sentMessage("car-1", tick = 3L, seq = 1L, receiverId = 9L)
    val m2 = sentMessage("car-1", tick = 3L, seq = 2L, receiverId = 5L)
    val m3 = sentMessage("car-1", tick = 4L, seq = 3L, receiverId = 9L)
  ...
  it should "not deduplicate repeated sends to the same receiver -- each is a distinct message identity" in {
    val toSameReceiverTwice = sentMessage("car-1", tick = 3L, seq = 1L, receiverId = 9L)
    val toSameReceiverAgain = sentMessage("car-1", tick = 3L, seq = 2L, receiverId = 9L)
